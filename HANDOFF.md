...
> 2026-09-06 晚修订：① 明确重启策略——能热更新（Vite HMR、ingest 单进程重启）就热更新，热更新不了（引擎侧/dochaus 配置层）就正常重启，不要因怕打断而僵着（见第 4 节）；② 踩坑表补中文文件名下载 500 与 OCR 测试 PATH 两行（见第 9 节）。
...
- 最近交付的是工作台紧凑列表、概况摘要、右侧编辑、连续确认、筛选批量操作及时间轴快捷录入，详见 `docs/case-workbench-compact.md`。此前手动案情录入新增 `tenure` 概况字段，详见 `docs/manual-intake.md`。
- 根目录当前**没有 `.git`**。不要假定可以 Git 回滚，不要执行破坏性清理。备份不是完整运行数据备份。
...
1. 读取用户本轮要求、本文件和 AGENTS。确认当前目录、`.git` 是否存在、WORKSPACE_ROOT 和正在监听的端口。
