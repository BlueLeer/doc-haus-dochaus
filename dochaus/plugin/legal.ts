...
import { readCasebook } from "../../services/ingest/src/casebook"
import { isAbsolute, relative, resolve, sep } from "node:path"
import { findQuote, liveText } from "../lib/extract"
import { formatCitations, type DocumentCitation } from "../lib/citations"
import { resolveJurisdictions, readMatterJurisdictions } from "../lib/jurisdiction"
...
export const LegalPlugin: Plugin = async (input) => {
  ...
  return {
    ...
    const requestedJurisdictions = readMatterJurisdictions(input.directory)
    const packs = await resolveJurisdictions(requestedJurisdictions)
    const resolvedCodes = new Set(packs.map((pack) => pack.code))
    const retired = requestedJurisdictions.filter((code) => !resolvedCodes.has(code))
    if (retired.length) {
      output.system.push(
        `<retired-jurisdiction>\n` +
          `This pre-migration matter retains retired jurisdiction binding(s): ${retired.join(", ")}. ` +
          `The matter and its documents remain accessible, but those legal data packs are no longer active. ` +
          `Do not substitute CN-CQ rules or any other jurisdiction. Tell the user that jurisdiction-specific ` +
          `legal conclusions require migration or separate verification.\n` +
          `</retired-jurisdiction>`,
      )
    }
  ...
  "tool.execute.before": async (input, output) => {
    ...
    // statute and regulation text (the labor-local-rules skill), not browse the
    ...
    throw new Error(
      `webfetch is limited to official primary legal sources (government and court sites, ` +
        ...
        `Use the sources in the labor-local-rules skill or the matter's jurisdiction pack. ` +
  ...
  "tool.execute.after": async (input, output) => {
    ...
    if (input.tool === "draft-document" && output.metadata?.document) {
      output.output +=
        `\n\n[draft-review] Before presenting this draft to the user, spawn the labor-reviewer ` +
