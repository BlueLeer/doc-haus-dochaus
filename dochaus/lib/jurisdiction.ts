...
const PRODUCT_FILE = path.join(import.meta.dir, "..", "product.json")

export async function productConfig(): Promise<ProductConfig> {
  return (await Bun.file(PRODUCT_FILE).json()) as ProductConfig
}
...
export async function resolveJurisdictions(codes: string[]): Promise<JurisdictionPack[]> {
  const resolved: JurisdictionPack[] = []
  const visiting = new Set<string>()
  const loaded = new Set<string>()
  const visit = async (code: string) => {
    if (loaded.has(code)) return
    if (visiting.has(code)) throw new Error(`jurisdiction inheritance cycle at ${code}`)
    visiting.add(code)
    ...
    if (pack.parent) await visit(pack.parent)
    visiting.delete(code)
    loaded.add(code)
    resolved.push(pack)
  }
  for (const code of codes) await visit(code)
  return resolved
}

export async function selectableJurisdictionCodes() {
  return new Set((await productConfig()).selectableJurisdictions)
}
