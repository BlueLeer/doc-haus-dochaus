...
const UNCONFIGURED =
  ...
  `(see the matter's jurisdiction pack <sources> and the labor-local-rules skill). ` +
...
export default tool({
  description:
    "Search the web to find a Chinese labor-law citation or official source. Results are leads, not authority: retrieve and verify the current official text with webfetch and the labor-local-rules skill before relying on it.",
  args: {
  ...
  async execute(args) {
    ...
    return {
      ...
      output:
        text +
        `\n\n[labor-local-rules] These results are leads, not authority. Before relying on any law they mention, ` +
