...
export default tool({
  ...
  args: {
    ...
    steps: tool.schema
      ...
      .describe(
        "Ordered list of active China labor-dispute subagents. Each step's output is passed to the next.",
      ),
  },
  async execute(args, ctx) {
    await ctx.ask({ always: [], permission: "create-workflow", patterns: [args.label], metadata: { workflow: args.label } })
