...
ordered pipelines of active China labor-dispute subagents that lawyers launch
on a case as one repeatable process.
...
- A workflow is a pipeline: each step's output is passed to the next. Order
  matters. In P1, use only subagents returned by `list-workflows`; the built-in
  legal review specialist is `labor-reviewer`.
