...
const matter = (await matterRes.json()) as { id: string; dir: string }
...
const approver = setInterval(async () => {
  ...
  if (!res?.ok) return
  const pending = (await res.json()) as Array<{ id: string; permission: string; metadata: Record<string, unknown> }>
