...
#   - poppler-utils + PaddleOCR + markitdown: PDF and scanned-document
#     ingestion. services/ingest shells out to markitdown (PDF -> Markdown) and
#     to pdftoppm + PaddleOCR (OCR for flat scans), so the
#     self-host image bakes them in.
RUN apt-get update && apt-get install -y --no-install-recommends \
    python3 python3-pip build-essential lsof procps libgl1 libgomp1 \
    poppler-utils \
  && rm -rf /var/lib/apt/lists/* \
  && pip3 install --no-cache-dir --break-system-packages "markitdown[pdf]" uv

WORKDIR /app
COPY . .
RUN bash services/ingest/scripts/setup-paddle-ocr.sh
