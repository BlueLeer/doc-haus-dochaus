...
export default function MatterDetail({ onSessionsChanged }: { onSessionsChanged: () => void }) {
  ...
  return (
    <>
        ...
        <div className="matter-head">
          {editingRef ? (
            ...
            <button
              ...
              title={t("Click to edit reference")}
              onClick={() => {
                setDraftRef(matter.reference ?? "")
                setEditingRef(true)
              }}
            >
              {matter.reference || t("Add reference")}
            </button>
          )}
          ...
          <JurisdictionSelect
            ...
            trigger={
              (matter.jurisdictions?.length ?? 0) === 0 ? (
                <span className="matter-jx-add">{t("Add jurisdiction")}</span>
              ) : (
      ...
      {matter.jurisdictions?.some((code) => jurisdictions.find((item) => item.code === code)?.rulesReady === false) && (
        <p className="settings-notice settings-warning">
          {t("Chongqing framework enabled — legal rules and local standards are not verified yet.")}
        </p>
      )}
      ...
      {view === "casebook" && <CaseWorkbench key={matter.id} matterId={matter.id} onView={setViewing} onExtract={() => setParams({
        view: "chat", agent: "labor", prompt: "请阅读本案件已上传材料，将案件概况、事实时间轴、证据目录和请求与证据关联整理到案件工作台，保存为待我确认的草稿。每条材料事实请附原文引用，双方冲突主张分别保留，不重复已有记录，未知信息列为待补充。",
      })} />}
            ...
            viewing.toLowerCase().endsWith(".docx")
              ? () => {
                  ...
                  setParams({
                    view: "chat",
                    agent: "labor-drafter",
                    prompt: `将“${doc}”保存为可复用模板。先完整读取文档，将当事人姓名、日期、金额、地址和其他案件特定信息分别替换为唯一且含义明确的【请填写……】占位符，再保存到模板库。`,
                  })
                }
              : undefined
          }
