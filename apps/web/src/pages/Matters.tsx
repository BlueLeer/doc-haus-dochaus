...
import { availableDefaultJurisdictions, loadPrefs } from "../prefs"
...
export default function Matters() {
  ...
  useEffect(() => {
    ...
    listJurisdictions().then((items) => {
      ...
      setNewJurisdictions(availableDefaultJurisdictions(items.map((item) => item.code)))
    })
  }, [])
  ...
  async function onCreate() {
    ...
    setTitle("")
    setReference("")
    setNewJurisdictions(availableDefaultJurisdictions(jurisdictions.map((item) => item.code)))
    setBusy(false)
    toast("success", t('Created matter "{title}".', { title: matter.title }))
  }
  ...
  async function onDelete(m: Matter) {
    ...
    toast("success", t('Deleted matter "{title}".', { title: m.title }))
  }
  ...
  async function onSaveEdit() {
    ...
    setEditing(null)
    setBusy(false)
    toast("success", t('Updated matter "{title}".', { title: updated.title }))
  }
  ...
  return (
    <>
      <header className="page-head">
        <div>
          <h1>{t("Matters")}</h1>
          <p className="page-sub">{t("Every engagement, its documents, and its conversations.")}</p>
        </div>
        <div className="page-head-actions">
          {matters.length > 0 && (
            <input
              placeholder={t("Filter by title or ID")}
              value={filter}
      ...
      <div className="card">
        ...
        <div className="row">
          <input
            placeholder={t("Matter title, e.g. Acme MSA review")}
            value={title}
          ...
          />
          <input
            placeholder={t("Matter ID (optional)")}
            value={reference}
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h3>{t("No matters yet")}</h3>
            <p>{t("A matter holds an engagement's documents, reviews, and conversations.")}</p>
          </div>
        ) : visible.length === 0 ? (
          <p className="muted">{t('No matters match "{filter}".', { filter })}</p>
        ) : (
            ...
            {visible.map((m) => (
              <li key={m.id} className="list-row">
                ...
                <div className="list-row-actions">
                  <button className="icon-btn" title={t("Edit matter")} onClick={() => openEdit(m)}>
                    {t("Edit")}
                  </button>
                  {confirmId === m.id ? (
                    <button
        ...
        <div className="viewer-overlay" onClick={() => setEditing(null)}>
          <div className="picker-panel matter-edit" onClick={(e) => e.stopPropagation()}>
            <div className="viewer-bar">
              <span className="viewer-title">{t("Edit matter")}</span>
