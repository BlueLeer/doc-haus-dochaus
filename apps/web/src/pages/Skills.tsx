...
export default function Skills() {
  ...
  async function onFiles(files: File[]) {
    ...
    if (!importable.length) {
      toast("error", t("Skills import .md, .txt, .docx, or .pdf files."))
      return
    }
    setBusy(true)
    for (const file of importable) {
      try {
        ...
        toast("success", t('Imported skill "{name}". Ask the skill builder to refine it.', { name: skill.name }))
      } catch (e) {
        toast("error", e instanceof Error ? e.message : t("Could not import {name}.", { name: file.name }))
      }
    }
    setBusy(false)
  }
  ...
  async function onDelete(s: Skill) {
    ...
    try {
      ...
      toast("success", t('Deleted skill "{name}".', { name: s.name }))
    } catch (e) {
      toast("error", e instanceof Error ? e.message : t("Could not delete {name}.", { name: s.name }))
    }
  }
  ...
  async function onToggle(s: Skill) {
    ...
    try {
      ...
      toast("error", e instanceof Error ? e.message : t("Could not update {name}.", { name: s.name }))
    }
  }
  ...
  return (
    <>
      <header className="page-head">
        <div>
          <h1>{t("Skills")}</h1>
          <p className="page-sub">{t("Reference playbooks the assistants can load.")}</p>
        </div>
      </header>

      <div className="card">
        <h2>{t("Import skill")}</h2>
        <div
          className="dropzone"
        ...
        >
          {busy
            ? t("Working...")
            : t("Drop a firm checklist or guide here (.md, .txt, .docx, .pdf), or click to choose a file.")}
        </div>
        <input
          ref={input}
          type="file"
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h3>{t("No skills yet")}</h3>
            <p>{t("Import one above, or describe one to the skill builder.")}</p>
          </div>
        ) : (
          <ul className="matter-list">
            {visible.map((s) => (
              <li
                ...
                title={t("View this skill")}
                onClick={() => setViewing(s)}
              >
                <div className="list-row-main">
                  <span className="list-row-title">
                    {s.name}
                    {s.builtin && <span className="badge badge-quiet">{t("Built-in")}</span>}
                  </span>
                  <span className="list-row-meta">
                    <span className="list-row-desc">{s.description || t("No description yet")}</span>
                  </span>
                </div>
                <div className="list-row-actions">
                  <button
                    ...
                    title={t(s.enabled ? "Disable skill — agents stop loading it" : "Enable skill")}
                    onClick={(e) => {
                      e.stopPropagation()
                      onToggle(s)
                    }}
