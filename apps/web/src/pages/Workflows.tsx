...
export default function Workflows() {
  ...
  async function onDelete(wf: CustomWorkflow) {
    ...
    toast("success", t('Deleted workflow "{name}".', { name: wf.label }))
  }
  ...
  return (
    <>
      <header className="page-head">
        <div>
          <h1>{t("Workflows")}</h1>
          <p className="page-sub">{t("Repeatable multi-agent review routines.")}</p>
        </div>
      </header>
          ...
          <ul className="matter-list">
            {WORKFLOWS.map((wf) => (
              <li
                ...
                title={t("View what this workflow does")}
                onClick={() => setViewing(wf)}
              >
                <div className="list-row-main">
                  <span className="list-row-title">
                    {t(wf.label)}
                    <span className="badge badge-quiet">{t("Built-in")}</span>
                  </span>
                  <span className="list-row-meta">
                    <span className="list-row-desc">{t(wf.description)}</span>
                  </span>
                </div>
                <div className="list-row-actions">
                  <button
                    className="icon-btn"
                    title={t("Launch this workflow in a matter")}
                    onClick={(e) => {
                      e.stopPropagation()
                      setUsing(wf)
                    }}
      ...
      {dir && (
        <ChatPanel
          ...
          title={t("Ask the workflow builder")}
      ...
      {using && (
        <MatterPicker
          title={t('Use "{name}" in a matter', { name: "created_at" in using ? using.label : t(using.label) })}
          ...
          <div className="picker-panel doc-panel" onClick={(e) => e.stopPropagation()}>
            <div className="viewer-bar">
              <span className="viewer-title">
                {"created_at" in viewing ? viewing.label : t(viewing.label)}
                {"created_at" in viewing ? "" : ` (${t("Built-in")})`}
              </span>
              <button onClick={() => setViewing(undefined)}>{t("Close")}</button>
            </div>
            <div className="picker-body">
              <p className="muted">{"created_at" in viewing ? viewing.description : t(viewing.description)}</p>
              <p className="muted">
                {viewing.scope === "matter"
                  ? t("Runs across every document in the matter, as its own conversation.")
                  : t("Runs on the document conversation you are already in.")}
              </p>
              <DocMarkdown>
                {[
                  viewing.steps?.length
                    ? `**${t("Pipeline")}**\n\n` +
                      viewing.steps.map((s, i) => `${i + 1}. **${s.agent}** — ${"created_at" in viewing ? s.instructions : t(s.instructions)}`).join("\n")
                    : "",
                  `**${t("Launch prompt")}**\n\n> ${"created_at" in viewing ? viewing.prompt : t(viewing.prompt)}`,
                ]
                  .filter(Boolean)
                  .join("\n\n")}
              </DocMarkdown>
