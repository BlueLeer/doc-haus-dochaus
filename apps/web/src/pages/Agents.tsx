...
export default function Agents() {
  ...
  async function onDelete(a: AgentRecord) {
    ...
    try {
      ...
      toast("success", t('Deleted agent "{name}".', { name: a.label }))
    } catch (e) {
      toast("error", e instanceof Error ? e.message : t("Could not delete {name}.", { name: a.name }))
    }
  }
  ...
  async function onToggle(a: AgentRecord) {
    ...
    try {
      ...
      toast("error", e instanceof Error ? e.message : t("Could not update {name}.", { name: a.name }))
    }
  }
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h3>{t("No agents yet")}</h3>
            <p>{t("Describe a specialist reviewer to the agent builder to add one.")}</p>
          </div>
        ) : (
          <ul className="matter-list">
            {visible.map((a) => (
              <li
                ...
                title={t("View this agent")}
                onClick={() => setViewing(a)}
              >
                <div className="list-row-main">
                  <span className="list-row-title">
                    {a.label}
                    {a.builtin && <span className="badge badge-quiet">{t("Built-in")}</span>}
                  </span>
                ...
                <div className="list-row-actions">
                  <button
                    ...
                    title={t(a.enabled ? "Disable agent — it leaves the workflow roster" : "Enable agent")}
                    onClick={(e) => {
                      e.stopPropagation()
                      onToggle(a)
                    }}
                  ...
                  <button
                    className="icon-btn"
                    title={t("View agent instructions")}
                    onClick={(e) => {
                      e.stopPropagation()
                      setViewing(a)
                    }}
