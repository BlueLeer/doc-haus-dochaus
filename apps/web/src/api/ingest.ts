...
import type { CasebookView, CaseRow } from "../../../../dochaus/lib/casebook-model"
export type { CasebookView, CaseRow, CaseField } from "../../../../dochaus/lib/casebook-model"

export async function getCasebook(id: string): Promise<CasebookView> {
  const response = await fetch(`${INGEST_URL}/matters/${id}/casebook`)
  if (!response.ok) throw new Error("无法读取案件工作台")
  return response.json()
}

export async function saveCasebook(id: string, revision: number, rows: CaseRow[]): Promise<CasebookView> {
  const response = await fetch(`${INGEST_URL}/matters/${id}/casebook`, {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "replace", revision, rows }),
  })
...
export type Jurisdiction = { code: string; name: string; citationStyle: string; rulesReady?: boolean }
...
export type UploadJob = {
  ...
  progress: { stage: "extracting" | "rendering" | "ocr" | "preparing" | "indexing" | "complete"; completed?: number; total?: number }
  error?: string; result?: IngestResult
}

export async function getUploadJobs(id: string): Promise<UploadJob[]> {
  const res = await fetch(`${INGEST_URL}/matters/${id}/uploads`)
  if (!res.ok) throw new Error("无法获取文件处理进度")
  return res.json()
}
...
export async function uploadDocument(id: string, file: File): Promise<IngestResult> {
  ...
  const res = await fetch(`${INGEST_URL}/matters/${id}/documents?async=true`, { method: "POST", body: form })
  if (!res.ok) {
    const message = (await res.text()).trim()
    throw new Error(message || `Could not index ${file.name} (${res.status})`)
  }
  const started: UploadJob = await res.json()
  while (true) {
    await new Promise((resolve) => setTimeout(resolve, 1000))
    // A lost progress connection is not a failed background extraction.
    // The component shows reconnecting while this loop waits for the service.
    const jobs = await getUploadJobs(id).catch(() => undefined)
    if (!jobs) continue
    const job = jobs.find((item) => item.id === started.id)
    if (!job) throw new Error("处理任务已不存在，服务可能已重启，请重新上传")
    if (job.status === "error") throw new Error(job.error || "文件处理失败")
