  ...
  const label =
    selected.length === 0
      ...
      : selected.length === 1
        ...
        : t("{count} jurisdictions", { count: selected.length })

  return (
    <div className="jx-select" ref={ref}>
      {trigger ? (
        <button type="button" className="jx-trigger-bare" onClick={toggleOpen} title={t("Click to edit jurisdiction")}>
          {trigger}
        </button>
      ) : (
        <button type="button" className="assistant-trigger jx-trigger" onClick={toggleOpen} title={t("Jurisdiction — steers reasoning and citation style")}>
        ...
        createPortal(
          ...
          >
            ...
            <div className="jx-list">
              {visible.length === 0 ? (
                <p className="muted jx-empty">{t("No match.")}</p>
              ) : (
                visible.map((j) => (
