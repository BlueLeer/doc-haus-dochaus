...
export default function ModelSelector({
  ...
  const triggerLabel = isAuto(current.name) && resolvedLabel ? `${t(current.label)} · ${t(resolvedLabel)}` : t(current.label)
  return (
    <>
      <button className="assistant-trigger" onClick={() => setOpen(true)} title={t("Choose assistant or workflow")}>
        ...
        <div className="viewer-overlay" onClick={() => setOpen(false)}>
          <div className="picker-panel picker-wide" onClick={(e) => e.stopPropagation()}>
            <div className="viewer-bar">
              <span className="viewer-title">{t("Choose an assistant")}</span>
            ...
            <div className="picker-body">
              {assistants.map((a) => (
                ...
                >
                  <span className="assistant-name">{t(a.label)}</span>
                  <span className="assistant-desc muted">{t(a.description)}</span>
                </button>
              ))}
            </div>
          </div>
