...
export default function WorkflowLauncher({
  ...
  return (
    <>
      <button className="workflow-btn" onClick={() => setOpen(true)} title={t("Run a multi-step workflow")}>
        ...
        <div className="viewer-overlay" onClick={() => setOpen(false)}>
          <div className="picker-panel" onClick={(e) => e.stopPropagation()}>
            <div className="viewer-bar">
              <span className="viewer-title">{t("Workflows")}</span>
            ...
            <div className="picker-body">
              {options.map((w) => (
                ...
                >
                  <span className="assistant-name">{t(w.label)}</span>
                  <span className="assistant-desc muted">{t(w.description)}</span>
                </button>
              ))}
            </div>
          </div>
