...
const SURFACES = [
  { view: "casebook", label: "案件工作台", Icon: IconReview },
...
export default function Sidebar({
  ...
  async function removeConvo(c: Convo) {
    ...
    try {
      ...
      notify("success", t("Conversation deleted"))
    } catch {
      notify("error", t("Could not delete conversation"))
    }
  }
    ...
    <aside className={`sidebar${collapsed ? " collapsed" : ""}`}>
      ...
      <nav className="sidebar-nav">
        <NavLink
          ...
          title={t("Matters")}
          aria-label={t("Matters")}
        >
          <IconMatters />
          {!collapsed && <span>{t("Matters")}</span>}
        </NavLink>
        <NavLink
          to="/templates"
          className={({ isActive }) => `nav-item${isActive ? " active" : ""}`}
        ...
        >
          <IconWorkflows />
          {!collapsed && <span>{t("Workflows")}</span>}
        </NavLink>
        <NavLink
          to="/skills"
          className={({ isActive }) => `nav-item${isActive ? " active" : ""}`}
      ...
      {matterId && !collapsed && view === "chat" && (
        <div className="sidebar-convos">
          <div className="sidebar-section-head">
            ...
            <Link to={`/matter/${matterId}?view=chat`} className="sidebar-new" title={t("New chat")}>
              <svg
                width="12"
                height="12"
                viewBox="0 0 24 24"
          ...
          {convos.length === 0 ? (
            <p className="muted sidebar-empty">{t("No conversations yet.")}</p>
          ) : (
