...
// Built-in labor agents use their Chinese product labels; a custom agent that
// is not in the registry still falls back to a readable identifier.
...
export default function ChatPanel({
  ...
  // "labor". The resolved agent drives the "Auto · <label>" chip and the
  ...
  async function resolveAgent(text: string, prior: Turn[]) {
    ...
    const history = prior
      ...
      .slice(-6)
      .map((t) => {
        const who = t.role === "user" ? "user" : `assistant (${t.agent ?? "labor"})`
    ...
    const resolved = await routeAgent({
      ...
      console.warn("Auto routing failed, falling back to labor assistant", err)
      return "labor"
    })
  ...
  async function onSend() {
    ...
    try {
      ...
      failTurn(translate(e instanceof Error ? e.message : "Could not reach the engine. Try again."))
    }
  }
  ...
  async function runWorkflow(name: string) {
    ...
    try {
      ...
      failTurn(translate(e instanceof Error ? e.message : "Could not start the workflow. Try again."))
    }
  }
  ...
  return (
    <div className="card chat-card">
      ...
      >
        ...
        {!loading && turns.length === 0 && !busy && (
          <div className="chat-empty">
            <p className="muted">{translate(emptyHint)}</p>
            <div className="starters">
              {starters.map((s) => (
                <button key={s} className="starter" onClick={() => setInput(translate(s))}>
                  <span className="starter-text">{translate(s)}</span>
        ...
        {turns.map((t, i) =>
          t.role === "user" ? (
            <div key={i} className="msg-user-wrap">
              ...
              {editing?.index !== i && t.id && !busy && (
                <div className="msg-actions">
                  ...
                  <button className="icon-btn" title={translate("Retry")} onClick={() => resendFrom(t, i, t.text)}>
                    <IconRetry />
                  </button>
                </div>
              )}
            ...
            <div key={i} className="msg assistant">
              {t.agent && <div className="msg-agent">{translate(agentLabel(t.agent))}</div>}
              ...
              {!t.error && isEmptyAnswer(t.text) && t.redlines.length === 0 && t.drafts.length === 0 && (
                <div className="msg-error">
                  {translate("No answer was produced for this question.")}{" "}
                  <button
                    className="linklike"
                    onClick={() => {
        ...
        {busy && (
          <div className={`msg assistant${live.text ? " streaming" : ""}`}>
            <div className="msg-agent">{translate(agentLabel(isAuto(agent) && resolvedAgent ? resolvedAgent : agent))}</div>
            <StepsPanel steps={live.steps} busy answered={Boolean(live.text)} />
            {live.text ? (
              <Markdown>{live.text}</Markdown>
            ) : (
              live.steps.length === 0 && (
                <span className="working" aria-label={translate("Working")}>
      ...
      <div className="composer">
        ...
        <div className="composer-footer">
          <span className="composer-hint" aria-hidden>
            {translate("Enter to send")}<span className="composer-hint-sep">·</span>{translate("Shift+Enter for a new line")}
          </span>
          <button className="send-btn" onClick={onSend} disabled={busy || !input.trim()} aria-label={translate("Send message")}>
      ...
      <p className="muted" style={{ marginTop: 10, fontSize: 12 }}>
        {translate("Not legal advice. AI can make mistakes.")}
        <br />
        {translate("Verify answers against the cited source before relying on them.")}
      </p>
    </div>
  )
}
...
function StepsPanel({ steps, busy, answered }: { steps: Step[]; busy: boolean; answered: boolean }) {
  ...
  return (
    <details className="steps" open={open} onToggle={(e) => setOpen(e.currentTarget.open)}>
      <summary>{thinking ? t("Working...") : t(steps.length === 1 ? "{count} step" : "{count} steps", { count: steps.length })}</summary>
...
function StepRow({ step, busy, last }: { step: Step; busy: boolean; last: boolean }) {
  const { t } = useI18n()
  const label = translateStepLabel(step.kind === "tool" ? step.label : "", t)
      ...
      <li className="step">
        ...
        {step.detail ? (
          <details className="step-detail">
            <summary className="step-label">{label}</summary>
  ...
  return (
    <li className="step">
      ...
      <details className="thinking" open={live}>
        <summary>{t(live ? "Analyzing..." : "Thought process")}</summary>
...
function translateStepLabel(label: string, t: (key: string, values?: Record<string, string | number>) => string) {
  const searchedDocument = label.match(/^Searched (.+) for "(.+)"$/)
  if (searchedDocument) return t('Searched {document} for "{query}"', { document: searchedDocument[1], query: searchedDocument[2] })
  const searched = label.match(/^Searched documents for "(.+)"$/)
  if (searched) return t('Searched documents for "{query}"', { query: searched[1] })
  const consulted = label.match(/^Consulted the (.+)$/)
  if (consulted) return t("Consulted the {role}", { role: t(consulted[1]) })
  const action = label.match(/^(Read|Wrote|Edited|Listed|Searched|Ran|Redlined) (.+)$/)
  if (action) return t("{action} {target}", { action: t(action[1]), target: action[2] })
  return t(label)
}
...
function PermissionCard({
  ...
  const detail =
    request.permission === "redact"
      ? t("Removes {count} occurrence(s) and any metadata mentions — this cannot be undone. Reason: {reason}", {
          count: String(meta.occurrences ?? t("all")),
  ...
  return (
    <div className="permission-card">
      <div className="permission-title">
        {t("The assistant wants to {verb} {document}", { verb: t(verb), document: document ?? t("a document") })}
      </div>
      {(oldText || newText) && (
      ...
      <div className="permission-actions">
        ...
        <button onClick={() => onReply(request.id, "always")}>{t("Always allow for this document")}</button>
