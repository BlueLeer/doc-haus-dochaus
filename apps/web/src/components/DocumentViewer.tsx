          ...
          <div className="viewer-bar">
            ...
            <div className="viewer-bar-actions">
              <button
                onClick={convert}
                disabled={converting}
                title={t("Convert this PDF to an editable Word document so it can be redlined")}
              >
                {t(converting ? "Converting..." : "Convert to DOCX")}
              </button>
    ...
    <div className="viewer-overlay" onClick={onClose}>
      <div className="viewer-panel" onClick={(e) => e.stopPropagation()}>
        <div className="viewer-bar">
          ...
          <div className="viewer-bar-actions">
            <button className="primary" onClick={download} disabled={!html} title={t("Download as a Word file with tracked changes")}>
              {t(redlines.length ? "Download redline" : "Download")}
            </button>
            {onSaveTemplate && (
              <button onClick={onSaveTemplate} title={t("Turn this document into a reusable template (with client details removed)")}>
                {t("Save as template")}
              </button>
            )}
            <button onClick={onClose}>{t("Close")}</button>
        ...
        <div className="viewer-body">
          <div className="viewer-doc">
            {wasmError && <p className="muted">{t("Viewer failed to load: {error}", { error: wasmError.message })}</p>}
            {error && <p className="muted">{error}</p>}
            {!wasmError && !error && !html && (
              <div className="docx-render docx-skeleton" aria-label={t("Rendering document")}>
            ...
            <aside className="changes-panel">
              <div className="changes-head">
                <span className="changes-count">
                  {t(redlines.length === 1 ? "{count} pending change" : "{count} pending changes", { count: redlines.length })}
                </span>
