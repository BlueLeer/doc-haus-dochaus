        ...
        <div className="settings-body">
          ...
          <div className="settings-content">
            {firstRun && tab === "providers" && (
              <p className="settings-notice">{t("Welcome to doc.haus. Connect a model provider — from a host sign-in (gcloud/AWS), an API key, or a local endpoint — then pick your default model under Models.")}</p>
            )}

            {tab === "models" && (
              <section className="settings-section">
                <h3>{t("Choose models")}</h3>
                <p className="settings-hint muted">{t("Pick a default and a fast model from one connected provider — both stay on that provider, so the Auto router (which runs the fast model) can never call across clouds.")}</p>
                {models.length === 0 && (
                  <p className="settings-hint muted">
                    {t("No provider is ready yet.")}{" "}
                    <button className="linklike" onClick={() => switchTab("providers")}>
                      {t("Connect one under Providers.")}
                    </button>
                  </p>
                )}
                <div className="row settings-row">
                  ...
                  <select value={provider} onChange={(e) => changeProvider(e.target.value)} disabled={models.length === 0}>
                    <option value="">{models.length ? t("Select a provider") : t("Add a provider first")}</option>
                ...
                <div className="row settings-row">
                  ...
                  <select value={smallModel} onChange={(e) => setSmallModelValue(e.target.value)} disabled={!provider}>
                    <option value="">{t("Same as default")}</option>
                    {providerModels.map((m) => (
                      <option key={m.value} value={m.value}>
                        {m.label}
                      </option>
            ...
            {tab === "providers" && (
              <>
                <section className="settings-section">
                  ...
                  <p className="settings-hint muted">{t("Model providers the engine already has credentials for — picked up from the server's sign-in (a gcloud or AWS login on the host). Nothing to set up; pick one as your default model under Models.")}</p>

                  <div className="settings-connected">
                    {readyProviders.length === 0 && needsSetup.length === 0 && disabled.length === 0 && (
                      <span className="muted">{t("None detected yet. Connect one below.")}</span>
                    )}
                    {readyProviders.map((p) => (
                      <div key={p.id} className="settings-conn">
                        <span className="settings-conn-name">{p.name}</span>
                        <span className="settings-conn-meta muted">
                          {t(statusLabel(p, methods))} · {t("{count} models", { count: Object.keys(p.models).length })}
                        </span>
                        {isGated(p.id) && (
                        ...
                        <button
                          role="switch"
                          aria-checked={true}
                          aria-label={t("Disable {name}", { name: p.name })}
                          className="switch on"
                    ...
                    {extraReady.map((id) => (
                      <div key={id} className="settings-conn">
                        ...
                        <span className="settings-conn-meta muted">{t("ready")}</span>
                        <button
                          role="switch"
                          aria-checked={true}
                          aria-label={t("Disable {name}", { name: nameMemo[id] ?? id })}
                          className="switch on"
                    ...
                    {disabled.map((id) => (
                      <div key={id} className="settings-conn off">
                        ...
                        <span className="settings-conn-meta muted">{t("disabled")}</span>
                        <button
                          role="switch"
                          aria-checked={false}
                          aria-label={t("Enable {name}", { name: nameByID.get(id) ?? nameMemo[id] ?? id })}
                          className="switch"
                ...
                {setupCards.length > 0 && (
                  <section className="settings-section">
                    <h3>{t("Needs setup")}</h3>
                    <p className="settings-hint muted">{t("Vertex and Bedrock sign in from the server (gcloud ADC / AWS), not an API key. Set the project or region, sign in on the server, then Enable to verify — until a live check passes they stay out of the model picker so you can't pick a provider that will fail on the first call.")}</p>
                    {setupCards.map((spec) => {
                  ...
                  <div className="row settings-row">
                    <select value={pick} onChange={(e) => setPick(e.target.value)}>
                      ...
                      {keyProviders.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.name}
                          {connected.has(p.id) ? ` · ${t("connected")}` : ""}
                        </option>
                      ))}
                    </select>
                    <PasswordInput
                      placeholder={pick && connected.has(pick) ? t("Replace key") : t("API key")}
                      value={key}
                      onChange={setKey}
                      disabled={!pick}
                    />
                    <button
                      ...
                      onClick={async () => {
                        ...
                        toast("success", t("Connected {name}.", { name }))
                        setPick("")
                        setKey("")
                        await load()
                      }}
                ...
                <LocalEndpoint
                  onAdd={async (input) => {
                    ...
                    toast("success", t("Added {name}. Restart the engine if its models don't appear.", { name: input.name }))
                    await load()
                  }}
                />
              </>
    ...
    <div className="settings-needs">
      ...
      {providers.length > 1 && (
        <span className="muted" style={{ fontSize: 12 }}>
          {t("One check covers {providers}.", { providers: providers.map((p) => p.name).join(` ${t("and")} `) })}
        </span>
      )}
      {spec.fields.map((f) => {
...
function LocalEndpoint({
  ...
  return (
    <section className="settings-section">
      <h3>{t("Local endpoint")}</h3>
      <p className="settings-hint muted">{t("OpenAI-compatible servers — LM Studio, Ollama, vLLM. The URL must be reachable from the machine running the engine, not the browser.")}</p>
      <div className="settings-grid">
