...
export default function ReviewGrid({ matterId, title, directory, documents }: { matterId: string; title: string; directory: string; documents: Document[] }) {
  ...
  function addColumn() {
    const question = window.prompt(t("New column question (e.g. What is the governing law?)"))?.trim()
    if (!question) return
  ...
  function renameColumn(id: string) {
    ...
    const question = window.prompt(t("Edit column question"), col?.question)?.trim()
    if (!question) return
  ...
  async function extract(docName: string, question: string): Promise<GridCellData> {
    ...
    await sendPrompt(client, session.id, "labor", `仅执行本次表格单元格提取，不创建工作台分析记录。文档：“${docName}”。问题：${question}。使用 search-document 检索原文，仅简短回答该问题，末行严格使用 Source: §实际章节 标记来源；未查到则明确写未找到，不编造章节。`)
  ...
  if (documents.length === 0)
    return <div className="empty-inline">{t("Add documents to this matter to build a review grid.")}</div>

  return (
    <div className="card review">
      <div className="row review-toolbar">
        <div>
          <h2>{t("Tabular review")}</h2>
          <span className="review-summary">
            {t("{documents} documents · {questions} questions", { documents: documents.length, questions: grid.columns.length })}
          </span>
        </div>
        <div className="row" style={{ gap: 8 }}>
          {running > 0 && <span className="review-running">{t("Extracting {count}", { count: running })}</span>}
      ...
      {grid.columns.length === 0 ? (
        <div className="empty-inline">{t('Add a column — a question asked of every document, e.g. "What is the governing law?"')}</div>
      ) : (
        <div className="review-scroll">
          <table className="review-table">
            <thead>
              <tr>
                <th className="review-doc-col">{t("Document")}</th>
                {grid.columns.map((col) => (
                  <th key={col.id}>
                    <div className="review-col-head">
                      <button className="review-col-q" onClick={() => renameColumn(col.id)} title={t("Edit question")}>
                        {col.question}
                      </button>
                      <button className="icon-btn review-col-x" onClick={() => deleteColumn(col.id)} title={t("Delete column")}>
                        ×
                      </button>
                    </div>
                  </th>
                        ...
                        {!cell ? (
                          ...
                          <div className="review-cell-body">
                            ...
                            <div className="review-cell-actions">
                              ...
                              {stale && <span className="review-stale" title={t("Question changed since this answer")}>{t("stale")}</span>}
                              <button className={`icon-btn review-lock${cell.status === "reviewed" ? " locked" : ""}`} title={t(cell.status === "reviewed" ? "Unlock" : "Mark reviewed")} onClick={() => toggleReviewed(key)}>
                                {t(cell.status === "reviewed" ? "Locked" : "Lock")}
                              </button>
                              {cell.status !== "reviewed" && (
                                <button className="icon-btn" title={t("Re-run this cell")} disabled={running > 0} onClick={() => fill([{ docName: doc.name, column: col }])}>
                                  {t("Refresh")}
                                </button>
                              )}
                            </div>
                ...
                {(detailCell.comments ?? []).map((c) => (
                  <div key={c.id} className="review-comment">
                    <div className="review-comment-meta">
                      <span className="muted">{date(c.at, { dateStyle: "short", timeStyle: "short" })}</span>
                      <button className="icon-btn" title={t("Delete comment")} onClick={() => deleteComment(c.id)}>
                        ×
                      </button>
                    </div>
