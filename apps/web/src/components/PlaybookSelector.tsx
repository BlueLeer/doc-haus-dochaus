  ...
  return (
    <>
      <button
        ...
        title={t("Firm playbook for this matter — review checks clauses against its positions")}
      >
        ...
        <span>{boundLabel ? t("Playbook: {name}", { name: boundLabel }) : t("Playbook")}</span>
      </button>
      {open && (
        <div className="viewer-overlay" onClick={() => setOpen(false)}>
          <div className="picker-panel picker-wide" onClick={(e) => e.stopPropagation()}>
            <div className="viewer-bar">
              <span className="viewer-title">{t("Playbook")}</span>
              <button className="modal-close" aria-label={t("Close")} onClick={() => setOpen(false)}>
            ...
            <div className="picker-body">
              <p className="muted" style={{ margin: "0 0 4px" }}>
                {t("A firm playbook is your standard positions and approved clause text. The review checks this matter's documents against it.")}
              </p>
              {playbooks.map((p) => (
                <button
                  key={p.name}
              ...
              >
                <span className="assistant-name">{t("No playbook")}</span>
                <span className="assistant-desc muted">{t("Skip playbook checks for this matter")}</span>
              </button>
            </div>
          </div>
        </div>
