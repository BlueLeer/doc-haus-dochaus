...
import { listMatters, createMatter, getMatter, renameMatter, deleteMatter, matterDir, listJurisdictions, listPlaybooks, updatePlaybook, deletePlaybook, PlaybookError, readProductConfig, WORKSPACE_ROOT } from "./matter"
...
// RFC 6266/5987: HTTP header values must be ASCII, so a Chinese filename cannot
// go raw into Content-Disposition (Bun throws TypeError on the header value and
// the download 500s). Serve an ASCII fallback plus a percent-encoded filename*
// that browsers prefer, so downloads keep their real name.
...
app.get("/matters/:id/casebook", (c) => {
  const dir = matterDir(c.req.param("id"))
  if (!existsSync(path.join(dir, "matter.json"))) return c.json({ error: "案件不存在" }, 404)
  return c.json(readCasebook(dir))
})

app.post("/matters/:id/casebook", async (c) => {
  const dir = matterDir(c.req.param("id"))
  if (!existsSync(path.join(dir, "matter.json"))) return c.json({ error: "案件不存在" }, 404)
  const input = await c.req.json<{ revision: number; rows?: unknown; mode?: string; text?: string }>()
  try {
    if (input.mode === "analysis") return c.json(addCaseAnalysis(dir, input.revision, input.text ?? ""))
    if (input.mode !== "replace" && input.mode !== "propose") return c.json({ error: "操作类型无效" }, 400)
...
app.get("/product", (c) => c.json(readProductConfig()))
...
app.get("/matters/:id/uploads", (c) => c.json(listUploadJobs(matterDir(c.req.param("id")))))
...
app.post("/matters/:id/documents", async (c) => {
  ...
  if (c.req.query("async") === "true") {
    try {
      return c.json(startUploadJob(dir, path.basename(file.name), (report) => ingestDocument(dir, path.basename(file.name), buffer, report)), 202)
    } catch (error) {
...
// Seed only assets present in the active product manifest. The optional
// Chongqing labor demo remains gated behind `start.sh --demo`.
const { seedPlaybooks } = await import("./seed")
seedPlaybooks()
writeDraftingPreferences(readDraftingPreferences())
...
// First-time OCR may exceed Bun's default idle timeout before any bytes are
// returned. Subsequent reads reuse the content-addressed extraction cache.
export default { port, hostname, idleTimeout: 255, fetch: app.fetch }
