...
writeFileSync(
  path.join(dir, "agent", "labor-reviewer.md"),
  '---\ndescription: "复核劳动争议分析"\nmode: subagent\n---\n\n复核分析。',
)
...
test("renderAgentMarkdown: frontmatter fields", () => {
  const md = wf.renderAgentMarkdown({
    ...
    scope: "matter",
    prompt: "Run it",
    steps: [{ agent: "labor-reviewer", instructions: "" }],
    created_at: 0,
  })
...
test("renderAgentMarkdown: pipeline step 1 seeds from matter", () => {
  const md = wf.renderAgentMarkdown({
    ...
    scope: "matter",
    prompt: "",
    steps: [{ agent: "labor-reviewer", instructions: "" }, { agent: "labor-reviewer", instructions: "" }],
    created_at: 0,
  })
...
test("renderAgentMarkdown: instructions inlined when present, omitted when blank", () => {
  const mdWith = wf.renderAgentMarkdown({
    ...
    scope: "matter",
    prompt: "",
    steps: [{ agent: "labor-reviewer", instructions: "Focus on indemnities." }],
    created_at: 0,
  })
...
test("renderAgentMarkdown: output sections per step plus bottom line", () => {
  const md = wf.renderAgentMarkdown({
    ...
    scope: "matter",
    prompt: "",
    steps: [{ agent: "labor-reviewer" }, { agent: "labor-reviewer" }],
    created_at: 0,
  })
  expect(md).toContain("**Labor Reviewer**")
  expect(md).toContain("**Labor Reviewer**")
...
test("collision: existing .md file (hand-authored labor-reviewer)", () => {
  expect(() =>
    wf.createWorkflow({
      label: "labor reviewer",
      description: "",
      scope: "matter",
      prompt: "",
      steps: [{ agent: "labor-reviewer" }],
  ...
  expect(existsSync(path.join(dir, "agent", "labor-reviewer.md"))).toBe(true)
  expect(readFileSync(path.join(dir, "agent", "labor-reviewer.md"), "utf8")).toContain("复核劳动争议分析")
})

test("collision: name already in registry", () => {
  wf.createWorkflow({ label: "My Flow", description: "", scope: "matter", prompt: "", steps: [{ agent: "labor-reviewer" }] })
  expect(() =>
    wf.createWorkflow({ label: "My Flow", description: "", scope: "matter", prompt: "", steps: [{ agent: "labor-reviewer" }] }),
  ).toThrow()
  // cleanup
...
test("collision: reserved name", () => {
  expect(() =>
    wf.createWorkflow({ label: "build", description: "", scope: "matter", prompt: "", steps: [{ agent: "labor-reviewer" }] }),
  ).toThrow()
})
...
test("label that slugs to empty → 400-mapped error", () => {
  let err: any
  try {
    wf.createWorkflow({ label: "---", description: "", scope: "matter", prompt: "", steps: [{ agent: "labor-reviewer" }] })
  } catch (e) {
    err = e
  }
  expect(err).toBeDefined()
...
test("delete of name not in registry throws, hand-authored .md untouched", () => {
  expect(() => wf.deleteWorkflow("labor-reviewer")).toThrow()
  expect(existsSync(path.join(dir, "agent", "labor-reviewer.md"))).toBe(true)
  expect(readFileSync(path.join(dir, "agent", "labor-reviewer.md"), "utf8")).toContain("复核劳动争议分析")
})
...
test("updateWorkflow throws 404 for unknown name", () => {
  let err: any
  try {
    wf.updateWorkflow("does-not-exist", { label: "x", description: "", scope: "matter", prompt: "", steps: [{ agent: "labor-reviewer" }] })
  } catch (e) {
    err = e
  }
  expect(err).toBeDefined()
