...
export function listSkills(): Skill[] {
  ...
  const active = existsSync(productFile)
    ? new Set((JSON.parse(readFileSync(productFile, "utf8")) as { activeSkills: string[] }).activeSkills)
    : undefined
  return [
    ...readSkillDir(BUILTIN_SKILL_DIR(), true).filter((skill) => !active || active.has(skill.name)),
