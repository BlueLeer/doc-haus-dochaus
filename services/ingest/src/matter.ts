...
// jurisdictions are pack codes (for P1, ["CN-CQ"]) the engine reads from
...
export function createMatter(title: string, reference?: string, jurisdictions?: string[], playbook?: string): Matter {
  const selected = requireSelectableJurisdictions(
    jurisdictions?.length ? jurisdictions : readProductConfig().defaultJurisdictions,
  )
  const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "matter"
...
export function renameMatter(id: string, title: string, reference?: string, jurisdictions?: string[], playbook?: string): Matter {
  const dir = matterDir(id)
  const existing = getMatter(id)
  const selected = jurisdictions?.length ? jurisdictions : readProductConfig().defaultJurisdictions
  const unchangedLegacy = JSON.stringify(selected) === JSON.stringify(existing.jurisdictions)
  const matter = {
    ...existing,
    title,
    reference,
    jurisdictions: unchangedLegacy ? selected : requireSelectableJurisdictions(selected),
    playbook,
  }
  writeFileSync(matterFile(dir), JSON.stringify(matter, null, 2))
  return matter
...
const PRODUCT_FILE = path.join(path.dirname(JURISDICTION_DIR), "product.json")

export type ProductConfig = {
  id: string
  name: string
  defaultJurisdictions: string[]
  selectableJurisdictions: string[]
...
export function readProductConfig(): ProductConfig {
  return JSON.parse(readFileSync(PRODUCT_FILE, "utf8")) as ProductConfig
}

function requireSelectableJurisdictions(jurisdictions: string[]) {
  if (!jurisdictions.length) throw new Error("at least one jurisdiction is required")
  const selectable = new Set(readProductConfig().selectableJurisdictions)
  const invalid = jurisdictions.filter((code) => !selectable.has(code))
  if (invalid.length) throw new Error(`jurisdiction is not available for new matters: ${invalid.join(", ")}`)
...
export function listJurisdictions() {
  ...
  return readdirSync(JURISDICTION_DIR)
    ...
    .map((file) => JSON.parse(readFileSync(file, "utf8")) as { code: string; name: string; citationStyle: string; rulesReady?: boolean })
    .filter((profile) => selectable.has(profile.code))
...
const PLAYBOOK_SOURCE_DIR = path.join(path.dirname(JURISDICTION_DIR), "playbooks")
...
export function listPlaybooks(): Playbook[] {
  ...
  const active = new Set(readProductConfig().activePlaybooks)
  const seededFile = path.join(PLAYBOOK_DIR, ".seeded")
  const seeded = new Set(existsSync(seededFile) ? readFileSync(seededFile, "utf8").split("\n").filter(Boolean) : [])
  return [
    ...
    .filter((p) => {
      if (p.builtin || active.has(p.name) || p.matters.length || !seeded.has(p.name)) return true
      const original = path.join(PLAYBOOK_SOURCE_DIR, p.name, "SKILL.md")
      const installed = path.join(PLAYBOOK_DIR, p.name, "SKILL.md")
      return !existsSync(original) || !existsSync(installed) || readFileSync(original, "utf8") !== readFileSync(installed, "utf8")
    })
    .sort((a, b) => a.name.localeCompare(b.name))
}
