...
export async function ingestDocument(matterDir: string, fileName: string, buffer: Buffer, report?: ReportProgress) {
  ...
  const extraction = normalizeExtractedText(await extractRawText(docPath, buffer, report))
  ...
  let chunkIndex = 0
  let flaggedChunks = 0
  const total = sections.reduce((sum, section) => sum + chunkSection(section).length, 0)
  report?.({ stage: "indexing", completed: 0, total })
  for (const section of sections) {
    for (const chunk of chunkSection(section)) {
      ...
      report?.({ stage: "indexing", completed: chunkIndex, total })
    }
  }
...
async function extractRawText(fileName: string, buffer: Buffer, report?: ReportProgress) {
  return cachedText(fileName, buffer, async () => {
    if (fileName.toLowerCase().endsWith(".pdf")) return pdfToText(buffer, report)
