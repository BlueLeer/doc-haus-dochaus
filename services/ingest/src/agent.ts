...
export function listAgents(): AgentSummary[] {
  ...
  const productFile = path.join(DOCHAUS_DIR(), "product.json")
  const active = existsSync(productFile)
    ? new Set((JSON.parse(readFileSync(productFile, "utf8")) as { activeAgents: string[] }).activeAgents)
    : undefined
  ...
  const builtins = !existsSync(AGENT_DIR())
    ...
    : readdirSync(AGENT_DIR())
        .filter((f) => f.endsWith(".md") && !customNames.has(f.slice(0, -3)) && (!active || active.has(f.slice(0, -3))))
        .flatMap((f) => {
