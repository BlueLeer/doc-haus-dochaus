...
> 开发接力：请先阅读 [项目接力文档 HANDOFF.md](HANDOFF.md)，了解重庆劳动争议改造现状、阶段规划、踩坑记录与尚未完成的能力。
...
[PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR) are installed), and ask.
Everything — conversations, documents, the index — persists on your machine.

本项目扫描件 OCR 默认使用本地 PaddleOCR PP-OCRv6 small 中文模型。安装 Poppler 和 uv 后，运行
`bash services/ingest/scripts/setup-paddle-ocr.sh` 安装独立 Python 环境及模型。
模型位于 `services/ingest/assets/paddleocr/`；运行识别时不下载模型、不上传案件文件，
也不会回退到系统 Tesseract。旧 Tesseract 全文缓存不会复用；既有检索索引如需
更新为 PaddleOCR 结果，需要重新上传对应文件。300 页等大文件仍需后续异步作业支持。
